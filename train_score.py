...
from model_def.tri_stream_gcn import TriStreamGCN


def parse_args():
    parser = argparse.ArgumentParser(description="训练三流GCN模型")
    parser.add_argument(
        "--work_dir",
        type=str,
        default="./work_dir/tri_stream",
        help="工作目录",
    )
...
class SkeletonFeeder(Dataset):
    ...
    def load_data(self):
        ...
        with open(self.label_path, "rb") as f:
            self.sample_name, self.label, self.accuracies = pickle.load(f)
...
def train(args):
    ...
    for epoch in range(start_epoch, args.max_epochs):
        ...
        for batch_idx, (data, _, accuracies, _) in enumerate(train_loader):
            data = data.to(device)  # [B, 14, 4, T, V]
            ...
            similarity_scores = similarity_scores.view(B, N)  # [B, 14]
